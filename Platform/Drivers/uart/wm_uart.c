...
/* 9600bps : 260 */
    {
        bd = (APB_CLK / (16 * 115200) -
              1) | (((APB_CLK % (115200 * 16)) * 16 / (115200 * 16)) << 16);
...
/* 9600bps : 260 */
    ...
    {
        bd = (APB_CLK / (16 * 115200) - 1) | (((APB_CLK % (115200 * 16)) * 16 / (115200 * 16)) << 16);
        tls_reg_write32(HR_UART0_BAUD_RATE_CTRL, bd);
    /* Line control register : Normal,No parity,1 stop,8 bits, only use tx */
...
int tls_uart_set_baud_rate_inside(struct tls_uart_port *port, u32 baudrate)
{
    ...
    value = (APB_CLK / (16 * baudrate) - 1) |
            (((APB_CLK % (baudrate * 16)) * 16 / (baudrate * 16)) << 16);
 ...
 * @retval
 */
void tls_uart_free_tx_sent_data(struct tls_uart_port *port)
{
    tls_uart_tx_msg_t *tx_msg = NULL;
...
int tls_uart_port_init(u16 uart_no, tls_uart_options_t * opts, u8 modeChoose)
{
	...
	if( uart_no==TLS_UART_0 || uart_no==TLS_UART_1 || uart_no==TLS_UART_2 )
	{
		port = &uart_port[uart_no];
		if (port->recv.buf != NULL)
		{
			TLS_DBGPRT_INFO("Uart %d already initialised before\r\n", uart_no);
			return WM_FAILED;
		}
	}
