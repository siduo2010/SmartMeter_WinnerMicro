...
#define FLASH_READ_BUF_SIZE   (256)
	...
	unsigned int sz_1k = sz/FLASH_READ_BUF_SIZE;
	unsigned int sz_remain = sz%FLASH_READ_BUF_SIZE;
	int i = 0;

	flash_addr = addr;
	for (i = 0; i < sz_1k; i++)
	{
		readByCMD(flash_addr, buf, FLASH_READ_BUF_SIZE, 0);
	...
	if (sz_remain)
	{
		readByCMD(flash_addr, buf, sz_remain, 0);
	}
