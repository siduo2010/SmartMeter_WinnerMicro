...
int tls_pwm_duty_config(u8 channel, u8 duty)
{
    ...
    if (4 == channel)
    {
        tls_reg_write32(HR_PWM_CH4_REG2, tls_reg_read32(HR_PWM_CH4_REG2) & ~0x0000FF00);          /* duty radio */
        tls_reg_write32(HR_PWM_CH4_REG2, tls_reg_read32(HR_PWM_CH4_REG2) | (duty << 8));
    }
    else
    {
        tls_reg_write32(HR_PWM_CMPDAT,   tls_reg_read32(HR_PWM_CMPDAT) & ~(0xFF << channel * 8)); /* duty radio */
        tls_reg_write32(HR_PWM_CMPDAT,   tls_reg_read32(HR_PWM_CMPDAT) |  (duty << channel * 8));
    }
...
int tls_pwm_freq_config(u8 channel, u16 clkdiv, u8 period)
{
    ...
    if (4 == channel)
    {
        tls_reg_write32(HR_PWM_CH4_REG1, tls_reg_read32(HR_PWM_CH4_REG1) & ~0xFFFF0000);/* clock divider */
        tls_reg_write32(HR_PWM_CH4_REG1, tls_reg_read32(HR_PWM_CH4_REG1) | (clkdiv << 16));

        tls_reg_write32(HR_PWM_CH4_REG1, tls_reg_read32(HR_PWM_CH4_REG1) & ~0x0000FF00); /* the number of the counting clock cycle */
        tls_reg_write32(HR_PWM_CH4_REG1, tls_reg_read32(HR_PWM_CH4_REG1) | ((period) << 8));
    }
    else
    {
        tls_reg_write32(HR_PWM_CLKDIV01 + (channel / 2) * 4, tls_reg_read32(HR_PWM_CLKDIV01 + (channel / 2) * 4) & ~(0xFFFF << ((channel % 2) * 16)));/* clock divider */
        tls_reg_write32(HR_PWM_CLKDIV01 + (channel / 2) * 4, tls_reg_read32(HR_PWM_CLKDIV01 + (channel / 2) * 4) |  (clkdiv << ((channel % 2) * 16)));

        tls_reg_write32(HR_PWM_PERIOD,   tls_reg_read32(HR_PWM_PERIOD) & ~(0xFF << channel * 8)); /* the number of the counting clock cycle */
        tls_reg_write32(HR_PWM_PERIOD,   tls_reg_read32(HR_PWM_PERIOD) |  ((period) << (channel * 8)));
    }
...
int tls_pwm_stoptime_by_period_config(u8 channel, u8 pnum)
{
    ...
    if (4 == channel)
    {
        tls_reg_write32(HR_PWM_CH4_REG1, tls_reg_read32(HR_PWM_CH4_REG1) & ~0x000000FF);
        tls_reg_write32(HR_PWM_CH4_REG1, tls_reg_read32(HR_PWM_CH4_REG1) | pnum);
    }
    else
    {
        tls_reg_write32(HR_PWM_PNUM,       tls_reg_read32(HR_PWM_PNUM) & ~(0xFF << channel * 8));
        tls_reg_write32(HR_PWM_PNUM,       tls_reg_read32(HR_PWM_PNUM) |  (pnum << channel * 8));
    }
...
int tls_pwm_deadzone_config(u8 channel, bool dten, u8 dtclkdiv, u8 dtcnt)
{
    ...
    if(ENABLE == dten)
    {
        tls_reg_write32(HR_PWM_DTCTL, tls_reg_read32(HR_PWM_DTCTL) & ~0x00030000);     /* dead zone clock divider */
        tls_reg_write32(HR_PWM_DTCTL, tls_reg_read32(HR_PWM_DTCTL) | (dtclkdiv<<16));

        if (channel == 0 || channel == 1)
        {
            tls_reg_write32(HR_PWM_DTCTL, tls_reg_read32(HR_PWM_DTCTL) & ~0x000000FF); /* the number of the counting clock cycle */
            tls_reg_write32(HR_PWM_DTCTL, tls_reg_read32(HR_PWM_DTCTL) | dtcnt);
        ...
        {
            tls_reg_write32(HR_PWM_DTCTL, tls_reg_read32(HR_PWM_DTCTL) & ~0x0000FF00); /* the number of the counting clock cycle */
            tls_reg_write32(HR_PWM_DTCTL, tls_reg_read32(HR_PWM_DTCTL) | (dtcnt<<8));
 ...
 * @param[in]      	pnum       is a pointer to period num,num range from 0 to 255
 *
 * @retval         WM_SUCCESS success
 * @retval         WM_FAILED  failed
 *
