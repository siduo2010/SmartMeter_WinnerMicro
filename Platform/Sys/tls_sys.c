...
#if PING_SELF_TEST
extern void tls_start_ping(void);

u8 ssidinfo[2][32] = {"lsd_sp01test_2", "lsd_sp01test_3"};
u8 gforcedisconnet = 0;
void tls_proc_ssid(u8 datanum)
{
	struct tls_param_ssid ssid;
	if (datanum < 3)
	{
		...
		ping_test_stop();
		tls_wifi_disconnect();
		tls_wifi_connect(ssidinfo[datanum - 1],strlen(ssidinfo[datanum - 1]),"1234567890",10);
	}	
}
#endif
...
#endif
#if PING_SELF_TEST
		  case SYS_MSG_UPDATE_SSID:
		  		tls_proc_ssid(*(u8 *)(msg->data));
		  		break;
#endif					
               default:
		            break;
...
#if PING_SELF_TEST
void tls_sys_proc_ssid_udpate(u8 num)
{
	u8 *data = NULL;
	data = tls_mem_alloc(1);
	*data = num;
	tls_sys_send_msg(SYS_MSG_UPDATE_SSID, (void*)data);
}
#endif
...
#define SYS_MSG_SIZE     20
    ...
    /* create task */
    tls_os_task_create(NULL, "Sys Task",
            tls_sys_task,
            (void *)0,
