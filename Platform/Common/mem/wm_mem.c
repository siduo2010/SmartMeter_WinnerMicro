...
#define MEM_HEAD_FLAG (0xAA55A55A)
#endif
...
void * mem_realloc_debug(void *mem_address, u32 size)
{
	...
	u32 cpu_sr;
    cpu_sr = tls_os_set_critical();
	if ((mem_re_addr = mem_alloc_debug(size)) == NULL)
	{
//		printf("mem_realloc_debug err\n");
		return NULL;
	}
	if(mem_address != NULL)
	{
...
//	buffer = (u32*)calloc(n,size);
	buffer = mem_alloc_debug(length);
	if(buffer==NULL)
	{
//		printf("mem_calloc_debug err\n");
	}
	tls_os_release_critical(cpu_sr);  
