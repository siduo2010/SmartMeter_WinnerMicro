...
static s8 hostif_socket_rpt(u8 skt_num, u16 datalen, u8 *ipaddr, u16 port, s8 err)
{
	...
	char *cmdind_buf = NULL;
	u32 cmdind_size = 0;
	struct tls_hostif_tx_msg *tx_msg = NULL;
	struct tls_hostif *hif = tls_get_hostif();

	if (hif->rptmode){		
		cmdind_buf = tls_mem_alloc(CMDIND_BUF_SIZE);
		if(cmdind_buf){
			tx_msg = tls_mem_alloc(sizeof(struct tls_hostif_tx_msg));
			if (tx_msg)
			{
				cmdind_size = sprintf(cmdind_buf,"+SKTRPT=%d,%d,%d,%d\r\n\r\n",skt_num, datalen, *((u32*)ipaddr),port);
				tx_msg->offset = skt_num;
				tx_msg->time = datalen;
				tx_msg->u.msg_event.buf = cmdind_buf;
				tx_msg->u.msg_event.buflen = cmdind_size;

				if(tls_wl_task_callback(&wl_task_param_hostif, (start_routine) hostif_socket_rpt_handle, tx_msg, 0)){
					tls_mem_free(cmdind_buf);
					tls_mem_free(tx_msg);
				}
				return 0;
...
u8  pSession_flag=0;
void tls_hostif_http_client_recv_callback(tls_http_session_handle_t session, char *data, u32 total_len, u32 data_len)
{
	#undef CMDIND_BUF_SIZE
	...
	{	
		...
		err1 = tls_hostif_process_cmdrsp(hif->hostif_mode, data, data_len);
		if (err1)
		{
			tls_mem_free(data);
		}
...
void tls_hostif_http_client_err_callback(tls_http_session_handle_t session, int err)
{
	#undef CMDIND_BUF_SIZE
	#define CMDIND_BUF_SIZE 64
	char *cmdind_buf = NULL;
...
int wjoin_proc(u8 set_opt, u8 update_flash, union HOSTIF_CMD_PARAMS_UNION *cmd, union HOSTIF_CMDRSP_PARAMS_UNION * cmdrsp){
    ...
    conn->res = 0;
    err = tls_cmd_join(cmd->wjoin.mode, conn);
    if (conn->res == 1) {
        /* wlan is linkup, return link information */
        memcpy(cmdrsp->join.bssid, conn->bssid, 6);
        cmdrsp->join.type = conn->type;
        cmdrsp->join.channel = conn->channel;
        cmdrsp->join.encrypt = conn->encrypt;
        cmdrsp->join.ssid_len = conn->ssid_len;
        memcpy(cmdrsp->join.ssid, conn->ssid, cmdrsp->join.ssid_len);
        cmdrsp->join.rssi = conn->rssi;
        cmdrsp->join.result = 1;
    } else {
...
int httpc_proc(u8 set_opt, u8 update_flash, union HOSTIF_CMD_PARAMS_UNION *cmd, union HOSTIF_CMDRSP_PARAMS_UNION * cmdrsp){
	tls_http_client_msg msg;
/*    if (set_opt) {
        if((cmd->httpc.url_len>255)||(cmd->httpc.data_len>512))
            return -CMD_ERR_INV_PARAMS;
    	memset(&msg, 0, sizeof(tls_http_client_msg));
    ...
    if (set_opt) {
        ...
        else
        {
            if((cmd->httpc.url_len>255)||(cmd->httpc.data_len>512))
                return -CMD_ERR_INV_PARAMS;
        	memset(&msg, 0, sizeof(tls_http_client_msg));
        	msg.param.url = (char *)cmd->httpc.url;
        	msg.method = (tls_http_verb_t)cmd->httpc.verb;
        	if(cmd->httpc.verb == WM_VERB_POST || cmd->httpc.verb == WM_VERB_PUT)
        	{
        		msg.data_len = cmd->httpc.data_len;
        		msg.send_data = (char *)cmd->httpc.data;
        	}
        	msg.recv_fn = tls_hostif_http_client_recv_callback;
        	msg.err_fn = tls_hostif_http_client_err_callback;
        	tls_http_client_post(&msg);
            cmdrsp->httpc.psession = msg.session;
        }
...
int txg_proc(u8 set_opt, u8 update_flash, union HOSTIF_CMD_PARAMS_UNION *cmd, union HOSTIF_CMDRSP_PARAMS_UNION * cmdrsp){
    ...
    if(set_opt){
        memcpy(tx_gain, cmd->txg.tx_gain, 29);
#if 1	
        for(i= 0; i<29; i++)
	{
		if ((tx_gain[i] > 36)||(tx_gain[i] < 16))
		{
			TLS_DBGPRT_INFO("TxGain err %d, %x\r\n", i, tx_gain[i]);
			return -CMD_ERR_INV_PARAMS;
		}

		tx_gain[i] = hed_rf_txgainmap[tx_gain[i]]; 		/*转换序号为实际增益值*/
	}
#endif		
//        if(ATCMD_OP_EP == update_flash)
		{
...
//			tls_efuse_write(TLS_EFUSE_TXGAIN_MCS_OFFSET, &tx_gain[12], 17);
	...
	{
		for (j = 16; j <= 36; j++){
			if (tx_gain[i] == hed_rf_txgainmap[j])
			{
				tx_gain[i] = j;
			}
		}
...
#endif		
        memcpy(cmdrsp->txg.tx_gain, tx_gain, 29);
    }
    return 0;
}
...
static struct tls_cmd_t  at_ri_cmd_tbl[] = {
...
#if TLS_CONFIG_AP
    { "SLIST", NULL, 0x19, 0, 0, slist_proc},
    ...
    { "APENCRY", NULL, 0xF, 1, 1,softap_encry_proc },
    { "APKEY", NULL, 0xF, 3, 3, softap_key_proc },
    { "APCHL", NULL, 0xF, 1, 2,softap_chl_proc},
    { "APWBGR", NULL, 0xF, 2, 2, softap_wbgr_proc},
    { "APNIP", NULL, 0xF, 1, 17, softap_nip_proc },            
#endif
#if TLS_CONFIG_WIFI_PERF_TEST
...
/*        int ret, verb;
        ...
        MEMCPY(cmd->httpc.url, uri, cmd->httpc.url_len);
        ...
        if(verb == VerbPost || verb == VerbPut){
            ...
            MEMCPY(cmd->httpc.url+cmd->httpc.url_len, tok->arg[2], cmd->httpc.data_len);
        }*/
    ...
    int ret, verb;
        ...
        if(ret)
            return -CMD_ERR_INV_PARAMS;
        cmd->httpc.verb = (u8)verb;
        if(verb == WM_VERB_POST || verb == WM_VERB_PUT){
            if(tok->arg_found != 3){
                return -CMD_ERR_INV_PARAMS;
            }
            cmd->httpc.data_len = strlen(tok->arg[2]);
        ...
        if(tok->arg_found > 1)
            return -CMD_ERR_INV_PARAMS;
        if(tok->arg_found == 1){
            if (strtohexarray(cmd->txg.tx_gain, 29, tok->arg[0]) < 0)
    		{
