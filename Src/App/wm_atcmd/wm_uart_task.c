...
//将串口接收引脚上拉，防止因无外部上拉时设备持续进入接收中断函数
static void rx_pin_pullup(enum tls_io_name rx_pin)
{
    if(rx_pin >= WM_IO_PB_00)
        tls_reg_write32(HR_GPIOB_DATA_PULLEN, tls_reg_read32(HR_GPIOB_DATA_PULLEN) & (~ BIT(rx_pin-WM_IO_PB_00)));
    else
        tls_reg_write32(HR_GPIOA_DATA_PULLEN, tls_reg_read32(HR_GPIOA_DATA_PULLEN) & (~ BIT(rx_pin)));
}
...
//    rx_pin_pullup(WM_IO_PA_05);  //use PA05 as uart0 rx pin as default
//    rx_pin_pullup(WM_IO_PB_11);  //use PB11 as uart1 rx pin as default
...
/* setting uart0 */    
    ...
    {
    //根据flash读取的参数配置串口寄存器 
        uart_opts.baudrate = 4800;//uart_cfg.baudrate;
    ...
    if (def_socket)
    {
        ...
        do
        {
            ...
            // printf("\nsocket err val\n");
                ...
                tls_timer2_stop();
                tls_timer2_start(uart, uart1_delaytime);
                return;
            }
            if (err == ERR_MEM)
            {
