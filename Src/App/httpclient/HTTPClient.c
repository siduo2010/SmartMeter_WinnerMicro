...
UINT32 HTTPClientGetInfo (HTTP_SESSION_HANDLE pSession, HTTP_CLIENT *HTTPClient)
{
    ...
    HTTPClient->http_status_code              = pHTTPSession->HttpHeadersInfo.nHTTPStatus;
    HTTPClient->request_body_length_sent      = pHTTPSession->HttpCounters.nSentBodyBytes;
    HTTPClient->response_body_length_received = pHTTPSession->HttpCounters.nRecivedBodyLength;
    HTTPClient->total_response_body_length    = pHTTPSession->HttpHeadersInfo.nHTTPContentLength;
    HTTPClient->http_state                    = pHTTPSession->HttpState;
