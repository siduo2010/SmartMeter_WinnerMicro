...
#if PING_SELF_TEST
#define         PING_ABORT_TIMER_DELAY     (30 * HZ)
#else
...
#if PING_SELF_TEST	
extern void tls_sys_proc_ssid_udpate(void);
#endif
...
#if PING_SELF_TEST	
	ping_test_running = FALSE;
    tls_os_timer_stop(ping_test_abort_timer);
    gucpingtest = 0;
#else
...
#if PING_SELF_TEST
void tls_ping_stop(void)
{
	tls_os_timer_start(ping_test_abort_timer);
}
void tls_start_ping(void)
{
	...
	gw[2] = nif->gw.addr>>8;
	gw[3] = nif->gw.addr;
	sprintf(para.host, "%d.%d.%d.%d", gw[3], gw[2], gw[1], gw[0]);
	para.cnt = 0;
	para.interval = 10; /*ms*/
