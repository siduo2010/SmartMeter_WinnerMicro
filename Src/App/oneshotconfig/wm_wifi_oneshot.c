...
static OS_STK *OneshotTaskStk = NULL;
...
#define TLS_ONESHOT_RETRY_TIME  15000*HZ/1000 
...
#define TOTAL_CHAN_NUM 14
static u8 airwifichan[TOTAL_CHAN_NUM]={0xF,0xF,0xF,0xF,0xF,0xF,0xF,0xF,0xF,0xF,0xF,0xF,0xF, 0xF};
...
static void wifi_change_chanel(u32 chanid)
{
	if (chanid <9){
		tls_wl_change_channel_info(chanid, 3);
	}else{
		tls_wl_change_channel_info(chanid, 2);
	}
#if TLS_CONFIG_QQLINK_MODE_ONESHOT
...
{
#if TLS_CONFIG_UDP_ONE_SHOT
	if(0 == gucConfigMode) // modify by houxf 160105
	{
...
void tls_oneshot_task_handle(void *arg)
{
    ...
    for(;;)
    {
		...
		chanCnt = 0;
		wifi_change_chanel(airwifichan[chanCnt++]);
		...
		{
			tls_os_timer_stop(gWifiHandShakeTimOut);
			tls_os_timer_change(gWifiHandShakeTimOut, TLS_ONESHOT_RETRY_TIME);
		}
            break;
            ...
            case ONESHOT_SWITCH_CHANNEL:
	         /*SWITCH CHANNEL*/
 	 	 wifi_change_chanel(airwifichan[chanCnt]);
		 ONESHOT_DBG("chan:%d,%d\n", chanCnt, airwifichan[chanCnt ]);
		 if (chanCnt >= (uctotalchannum - 1))
		 {
			 chanCnt = 0;
		 }else{	 
 		 	chanCnt++;
...
{
	if (NULL == OneshotTaskStk){
		OneshotTaskStk = (OS_STK *)tls_mem_alloc(sizeof(OS_STK)*ONESHOT_TASK_SIZE);
		if(OneshotTaskStk == NULL)
		{
			return;
		}
		memset(OneshotTaskStk, 0, sizeof(OS_STK)*ONESHOT_TASK_SIZE);
...
void tls_wifi_set_oneshot_flag(u8 flag)
{
	if (1 == flag)
	{
		//if (guconeshotflag != 1)
		{
			...
			guconeshotflag = flag;
			//tls_restore_bcast_filter_mac();
			tls_wifi_disconnect();
			if (1== gucConfigMode)/*ap mode*/
			{
				tls_wifi_set_listen_mode(0);
			}
			else /*udp mode*/
